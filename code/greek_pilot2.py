"""Greek pilot 2 — gamma (point-eval), digital (step), barrier (truncated, discontinuous).
Tests whether Thm 3.1 folded-normal holds beyond Lipschitz payoffs. Read-only on paper.
Same cross_class NN ensemble (200 seeds, exact paper pair lambdas), theory+empirical same seeds.
"""
...
def payoffs(y, dy):
    ...
    def spike(y0):
        g = np.zeros(ny); g[int(np.argmin(np.abs(y-y0)))] = 1.0/dy; return g   # point-eval: <g,p>dy = p(y0)
    return [
        ("price_ATMcall",  np.maximum(np.exp(y)-1.0, 0.0)),        # ref (Lipschitz)
        ("delta_ATM",      np.exp(y)*(y > 0.0)),                    # ref (Lipschitz)
        ("gamma_ATM",      spike(0.0)),                            # (a) point-eval p(0)
        ("gamma_OTM110",   spike(lk)),                             # (b) point-eval p(log1.10)
        ("digital_ATM",    (y > 0.0).astype(float)),               # (c) step  1{y>0}   DISCONTINUOUS
        ("digital_OTM110", (y > lk).astype(float)),                # (d) step  1{y>log1.1} DISCONTINUOUS
        ("barrier_uo130",  (np.exp(y)-1.0)*((y > 0.0) & (y < lB))),# (e) up-and-out call, knockout at B=1.30 DISCONTINUOUS
    ]
