"""
b4_fig5_table4.py — B.4: Table 4 + Fig 5 재생성 (K_eta 기반, unified formulation)
====================================================================================

Loads B.2 results (NeuMoR/output/unified/*.npz) and produces:
  - Table 4: kernel structure (lambda_max, tr, r_eff, eigenvalue shares)
  - Fig 5:   r_eff(K_eta) vs SNR² gain scatter (8 points, Heston/Kou)

Issue 2 check: within-model r_eff vs gain — Pearson + Spearman.
  both > 0.5  → resolved (strong positive)
  both > 0    → weakly resolved
  any < 0     → not resolved

Outputs:
  NeuMoR/output/unified/b4_table4_kernel_structure.csv
  NeuMoR/figures/b4_fig5_reff_vs_gain.png
  NeuMoR/pdfs/b4_fig5_reff_vs_gain.pdf

Run:
  python NeuMoR/code/b4_fig5_table4.py
"""
...
for model in ["heston", "kou"]:
    for pair in ["A", "B", "C", "D"]:
        ...
        eigvals_eta = npz["eigvals_eta"]          # descending, clipped >= 0
        ...
        # sanity: r_eff matches summary_b2
...
# ── Within-model correlation (Issue 2) ───────────────────────────────────────
...
# Note: K_11 uses same seeds for l1 only, r_eff(K_11) same for all Heston pairs
...
# ── Fig 5: r_eff(K_eta) vs gain scatter ──────────────────────────────────────
...
# Range annotation box (top-left, inside plot)
