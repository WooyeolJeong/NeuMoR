"""
b5_thm35_387.py — B.5: Thm 3.5 Validation (387 configs, K_eta unified formulation)
====================================================================================

§5.2 Table 3의 모든 configs를 K_eta + d_hat 통일 formulation으로 재측정.
Ratio = empirical / theoretical mean |Δ| 가 [0.7, 1.3] 에 드는지 확인.

Config blocks:
  Exp01  (15):  Heston pairs A-E × 3 smooth payoffs
  Exp02  (39):  13 pairs (A-E, F1-F5, G1-G3) × 3 payoffs
  Exp03  (90):  6 protocol pairs × pairs A-E × 3 payoffs
  Exp05  (75):  5 axes × 5 pairs × 3 payoffs
  Exp05d (90):  6 diagonals × 5 scales × 3 payoffs
  Exp06  (24):  8 edge pairs × 3 payoffs
  Exp09  (12):  4 BTC windows × 3 payoffs
  KouAB  (24):  pairs A-D × 3 payoffs × 2 seed regimes (N=30 / N=200)
  KouBTC (18):  6 BTC scenarios × 3 payoffs
  TOTAL  (387)

Cross-check sanity:
  B.5 BTC configs (Exp09 + KouBTC) vs B.6 (b6_btc_thm35.csv):
  Both passing [0.7,1.3] → B.5 and B.6 self-consistent.
  (Direct config-level match not possible: different (λ1,λ2) definitions.)

Outputs:
  NeuMoR/output/unified/b5_thm35_all.csv     — all config rows
  NeuMoR/output/unified/b5_thm35_summary.csv — per-block summary (count, range, pass%)

Run:
  python NeuMoR/code/b5_thm35_387.py
"""
...
def load_heston_protocol(proto: str, n_seeds: int = None):
    """Load Heston models for a given protocol.

    Protocol A: seeds 0-49 from exp01, 50-199 from exp07_5.
    Protocol B1/B2/B3: seeds from exp03/protocol_Bx (30 seeds each).
    """
...
def measure_block(
    ...
    """Run Thm 3.5 for all 3 smooth payoffs given full inference arrays."""
    dy = float(y[1] - y[0])
    l1_cal = l1_all[:n_cal]
    l2_cal = l2_all[:n_cal]
...
def main():
    ...
    # ── Load Protocol A models (200 seeds) ────────────────────────────
    ...
    # ── Exp 03: 6 protocol pairs × 5 pairs × 3 payoffs = 90 ──────────
    ...
    # Protocol pairs: (proto for l1, proto for l2, n_seeds, n_cal, test_lo, test_hi)
    PROTO_PAIRS = {
    ...
    for pp_name, (p1_proto, p2_proto, n_seeds, n_cal, tlo, thi) in PROTO_PAIRS.items():
        for pid, (params1, params2) in EXP03_PAIRS_AE.items():
            # Infer using respective protocols
            if p1_proto == "A":
    ...
    # ── Exp 05 multi-axis: 5 axes × 5 pairs × 3 payoffs = 75 ─────────
    ...
    axis_pair_ids = ["Av", "Bv", "Cv", "Dv", "Ev"]  # placeholder names
    ...
    # ── Exp 05 diagonal: 6 diagonals × 5 scales × 3 payoffs = 90 ─────
    ...
    # ── Exp 06 edge: 8 pairs × 3 payoffs = 24 ────────────────────────
    ...
    # ── Exp 09 BTC: 4 windows × 3 payoffs = 12 ───────────────────────
    ...
    for wname, p1 in EXP09_PARAMS.items():
        ...
        if v2 > 0.300 or abs(v2 - p1["v"]) < 1e-9:
            v2 = p1["v"] - 0.01  # at upper boundary: perturb downward
    ...
    }  # Pairs A-D (in-scope); E excluded

    # ── KouAB: stageA regime (N=30 seeds), in-scope pairs A-D × 3 payoffs = 12 ──
    ...
    # ── KouAB: stageB regime (N=200 seeds), in-scope pairs A-D × 3 payoffs = 12 ──
    ...
    # ── KouBTC: 6 scenarios × 3 payoffs = 18 ─────────────────────────
