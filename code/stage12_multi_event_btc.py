"""
stage12_multi_event_btc.py — Stage 1+2: Multi-Event BTC Validation (§5.4 Extension)
=====================================================================================

8 BTC events × pre/post = 16 windows.
Stage 1: Heston MoM + Kou BNS/Lee-Mykland (η pooled from full history)
Stage 2: NN inference (200 seeds) + Thm 3.5 ratio

Run:
  python NeuMoR/code/stage12_multi_event_btc.py
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 1: Heston calibration (16 windows)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 2: Kou σ_diff (5-min BNS, 16 windows)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 3: Kou η pooling (Path A) + per-window λ_J
# ══════════════════════════════════════════════════════════════════════════════
...
# Step 3.2: per-window λ_J
lam_J_map = {}   # (event, window, c) → float
for event, window, d0, d1 in WINDOWS:
    t0=pd.Timestamp(d0,tz="UTC"); t1=pd.Timestamp(d1+" 23:59:59",tz="UTC")
    sub_d = log_daily_full[t0:t1]; n_d=len(sub_d)
    T_w = n_d/252.0
    # align sigma_local indices
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 4: c selection via validation
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 5: Build final calibration CSVs
# ══════════════════════════════════════════════════════════════════════════════
...
for event,window,d0,d1 in WINDOWS:
    ...
    nj  = int(np.sum(np.abs(log_daily_full[pd.Timestamp(d0,tz="UTC"):
              ...
              if False else []) > best_c))  # placeholder; computed below
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 8: NN Inference (load models once, run per window)
# ══════════════════════════════════════════════════════════════════════════════
...
# Run inference: pdfs[(event,window,model)] = shape (200, n_y)
pdfs = {}
for event,window,d0,d1 in WINDOWS:
    # Heston
    h = heston_cal.get((event,window))
    ...
    # clip to training box before inference
...
# ══════════════════════════════════════════════════════════════════════════════
# Sanity check vs existing b6_btc_thm35.csv
# ══════════════════════════════════════════════════════════════════════════════
