"""mix_estimator.py — 추정기 클래스 혼합(NN+MC)으로 straddle 실현 시도.
경로A: 디지털 행사가 창 k*_NN vs k*_MC (창 내 부호 반전). 경로B: Case C 스캔(bias_removal).
실현 시 naive 과소예측(->sqrt(pi/2)=1.2533)+층화 복원. within-seed D. 전부 캐시(추론 0). paper read-only.
FN(mu,sig)=sig*sqrt(2/pi)e^{-s^2/2}+mu*erf(s/sqrt2), s=mu/sig.
cal/eval 고정(셔플 없음): NN Heston 140/60·Kou 119/51, MC 둘다 140/60. 풀=각 앞30 full, eval=각 eval앞10.
"""
...
def ddens(cls,est,pr):  # per-seed density-diff base-partner (nseed,grid)
...
def Dvec(cls,est,pr,g,sl):  # per-seed Δ̂ over slice
    return ddens(cls,est,pr)[sl]@g*DATA[cls]["dy"]

# ===== Part 1: 편향 · 영점 지도 =====
print("=== Part1a: 추정기 편향 (μ-Δ_true)/σ  [cal] ===")
...
print("\n=== Part1b: 디지털 영점 k*_true/k*_NN/k*_MC (0.002 grid, cal) ===")
...
print("  |k*_NN-k*_MC| 순위:",", ".join(f"{cls}{pr}={g:.3f}" for g,cls,pr in rank[:6]))

# ===== Part 2A: 창 내 straddle 후보 (cal) =====
print("\n=== Part2A: straddle 창 후보 (|k*_NN-k*_MC|>=0.004, cal 부호반전) ===")
...
for (cls,pr),(kt,kn,km) in ZMAP.items():
    if abs(kn-km)<0.004 or np.isnan(kn) or np.isnan(km):
        print(f"  {cls}{pr}: gap={abs(kn-km):.4f} <0.004 -> 창 없음"); continue
    ...
    print(f"  {cls}{pr}: 창[{lo:.3f},{hi:.3f}] 부호반전셀 {len(cells)}개; picks: "+(", ".join(f"k={c2['k']:.3f}(sNN={c2['zN']:.2f},sMC={c2['zM']:.2f})" for c2 in pick) or "없음"))
...
print("\n=== Part3A: 풀 검정 (헤드라인 eval n=20, full n=60 병기) ===")
...
# ===== Part 4 =====
LIM=np.sqrt(np.pi/2)
print(f"\n=== Part4: 요약 (sqrt(pi/2)={LIM:.4f}) ===")
if p3:
    top=max(p3,key=lambda r:r["pred_gap"] if r["pred_gap"]==r["pred_gap"] else -9)
    print(f"  최대 pred_gap(eval): {top['cls']}{top['pair']} k={top['k']:.3f} pred_gap={top['pred_gap']:.3f}[{top['pg_lo']:.2f},{top['pg_hi']:.2f}] r_naive={top['r_naive']:.3f}[{top['rn_lo']:.2f},{top['rn_hi']:.2f}] r_strat={top['r_strat']:.3f} (도달률 {top['pred_gap']/LIM*100:.0f}% of {LIM:.3f})")
    print(f"    full 병기: pred_gap_full={top['pred_gap_full']:.3f} r_naive_full={top['r_naive_full']:.3f} sNN_f={top['sNN_f']:.2f} sMC_f={top['sMC_f']:.2f}")
    rz=[r for r in p3 if r["strad_eval"]==1];print(f"  eval straddle 실현: {len(rz)}/{len(p3)} 셀")
    print(f"  층화 r_strat 범위: [{min(r['r_strat'] for r in p3):.3f},{max(r['r_strat'] for r in p3):.3f}]")
else: print("  후보 0 — Part3 생략")
print(f"  Case C: {len(CASEC)}건")
print(f"\nTOTAL {time.time()-t0:.0f}s CSV -> {OUT} ({len(allrows)}행)")
