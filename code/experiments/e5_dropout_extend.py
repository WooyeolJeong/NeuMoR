"""
E5(ii) extend: seeds 0-19 (train 3-19 skip-existing) -> 640-cell diagnostics
+ seed-level prevalence + M/M/1 deployment-rule cross-table.
Reuses e5_dropout_run (train_seed/load_drop/mc_sample) and e2_width_run (protocol A, thm35_ratio).
Deployment rule verbatim from code/mm1_Tladder.py: atom(L54), rule(L124-126, kurt fisher=False L58).
PRESERVES the earlier e5_dropout_diagnostics.csv (96) and existing npy (loaded, not overwritten).
Writes only new files: e5_dropout_diagnostics_n20.csv, e5_seed_prevalence.csv.
"""
import sys, csv
...
def main():
        ...
        for X in "ABCD":
            ...
            for coupled in (True, False):
                ...
                if f1.exists() and f2.exists():
                    l1 = np.load(f1); l2 = np.load(f2)       # reuse earlier npy (seeds 0-2)
                else:
                ...
                for pn, g in pf.items():
                    dh = dpdf @ g * dy
                    kt = float(kurtosis(dh, fisher=True))      # excess (same convention as the earlier run)
                    kff = float(kurtosis(dh, fisher=False))    # normal=3 (mm1 rule convention)
                    sk = float(skew(dh))
                    try:
    ...
    # ---- prevalence (coupled, seed x pair) ----
    prev = []
    for s in SEEDS20:
        for X in "ABCD":
    ...
    # ---- deployment-rule cross-table (coupled only) ----
