"""
E4 follow-up: apply M/M/1 deployment rule (mm1_Tladder.py verbatim) to the 16 Burgers cells.
atom = mean(|D| <= 1e-15*max(1,|mu|)) with mu=D.mean(); alarm = atom>0.10 OR (kurt_nf>5 AND |s|<1.5).
kurt_nf = kurtosis(fisher=False); |s| = abs_s from e4_burgers_diagnostics.csv (boundary-suite, cal 0-39).
Read+compute only. Writes e4_deployment_rule.csv (16 rows). Does not modify existing E4 files.
"""
import sys, csv
...
# --- existing diagnostics: rej_lil + abs_s per cell ---
diag = {}
...
# --- rule (mm1_Tladder.py L54, L124-126 verbatim) ---
def atom_frac(D):
    mu = D.mean()
...
# which cells alarm / which reject
