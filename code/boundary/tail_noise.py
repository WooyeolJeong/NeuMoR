...
pT1=infer(models,[0.31,45,0.5,3.5,10.0],yg,lo,hi)
pT2=infer(models,[0.30,45,0.5,4.0,10.0],yg,lo,hi)
...
print("\n=== Part4a: noise variance Var(eta(y)) share by |y| band (tail shift?) ===")
print(f"{'cell':16s}{'|y|<=1':>9s}{'1-2':>9s}{'2-3':>9s}{'>3':>9s}")
band4a=[]
for cell,p1,p2 in CELLS:
    eta=(p1-p2)-(p1-p2).mean(0); v=eta.var(0)
    tot=v.sum()
...
print("\n=== Part4b: pointwise eta(omega,y) diagnostics (tail grid points, per-seed skew/kurt/KS) ===")
