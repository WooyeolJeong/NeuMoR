...
print("=== Part0: smoke (single run T=100000, burn 10000, a=0.70) ===")
...
if np.abs(pi0-pia).max()>1e-2:
    print("  [gate failed] simulator mismatch - aborting"); sys.exit(1)
...
print(f"\n=== Part1: 200 seeds x 11 runs done {time.time()-t0:.0f}s  maxN observed=[{min(mxs)},{max(mxs)}]"+(" WARN >120" if max(mxs)>120 else "")+" ===")
...
print(f"\n  s spectrum: |s| range [{min(alls):.2f},{max(alls):.2f}]  (>10 all cells? {all(x>10 for x in alls)})")
if all(x>10 for x in alls):
    print("  all cells |s|>10 -> adding da=0.001 partner (to reach the folding-active regime)")
...
print("\n=== Part3: Thm4.1 (CRN vs independent) ===")
...
print("\n=== Part4: initialization bias (pair D da=0.05 CRN, burn 1000 vs 0) ===")
def crn_pairD(burn):
