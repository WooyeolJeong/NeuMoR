...
NSAMP = 1000
LSTAR = (0.02, 1.0, 0.0)
DNU = [0.0005, 0.001, 0.002, 0.005]
SEEDS = list(range(50)); N_CAL = 40
...
def build_model():
    ...
    class DON(nn.Module):
        ...
        def forward(self, lam):
            return self.branch(lam) @ self.trunk.T
    return DON()
...
def diagnose(c, dx, w):
    ...
    for pn, dnu in zip("ABCD", DNU):
        ...
        for qn in ["L1", "L2", "M", "E"]:
            ...
            mu = float(cal.mean()); sig = float(cal.std(ddof=1))
