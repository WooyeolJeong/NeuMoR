...
def phase_diagnose(y_grid):
    ...
    for w in WIDTHS:
        for X in "ABCD":
            l1 = np.load(PDFS / f"w{w}_pair{X}_l1.npy").astype(np.float64)
            ...
            for pn, g in pf.items():
                ...
                ad_rej = bool(ad.statistic > ad.critical_values[2])
...
def main():
    t0 = time.time()
    ds = HestonDataset()
    y_grid = np.load(YGRID_NPZ)["y_grid"].astype(np.float64)
