...
class DensityMLP(nn.Module):
    ...
    def forward(self, lam: torch.Tensor, y_grid: torch.Tensor) -> torch.Tensor:
        ...
        x_flat  = torch.cat([lam_exp, y_exp], dim=-1)
...
h_lam = torch.tensor(h_npz["lambdas"], dtype=torch.float32)
h_pdf = torch.tensor(h_npz["pdfs"],    dtype=torch.float32)
h_y   = torch.tensor(h_npz["y_grid"], dtype=torch.float32)
...
k_lam = torch.tensor(k_npz["lambdas"], dtype=torch.float32)
k_pdf = torch.tensor(k_npz["pdfs"],    dtype=torch.float32)
k_y   = torch.tensor(k_npz["y_grid"], dtype=torch.float32)
...
b3_ref = pd.read_csv(ROOT / "release/data/suite/snr_gain_asymptotic.csv")
