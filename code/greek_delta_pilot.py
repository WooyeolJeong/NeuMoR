"""Delta model-risk bias pilot — folded-normal (Thm 3.1) with payoff swap g_delta.
Read-only on paper. Uses cross_class NN ensemble raw per-seed densities (exact paper pair lambdas).
Theory + empirical computed from the SAME 200-seed ensemble (self-consistent ratio).
"""
...
for model, spec in SPECS.items():
    ...
    M, LAM = E["matrix"], E["lambdas"]          # (S, C, ny), (C,5)
    y = y_from_pair(model); dy = float(y[1]-y[0]); ny = len(y)
    assert M.shape[2] == ny, (M.shape, ny)
    # base tail for kou: common tail among the sigma-perturbed configs
    if spec["base_tail"] is None:
        tails = [LAM[find_cfg(LAM,[v]+list(LAM[find_cfg(LAM[:,0:1]==v if False else None,None)] ) )] for v in []]  # placeholder
    base_tail = spec["base_tail"]
    if base_tail is None:
        # auto: gather rows whose col0 == a pair sigma, take the modal tail
    ...
    for pair, v2 in spec["pairs"].items():
        ...
        for pname, g in payoffs(y):
            ...
            dhat_seed = (p1 - p2) @ g * dy       # (S,) per-seed Delta_hat
            ...
            # tail decomposition (contribution to signal and to sigma^2 by y-band, on support of g)
            thr = 0.0 if pname != "delta_OTM1.10" else np.log(1.10)
            sig_i = g * d_hat * dy               # signal contribution per point
            var_i = g * Kg * dy**2               # sigma^2 contribution per point (partitions total)
