"""Part 1 — parameter-distance ladder (Heston, NN cache 200 seeds).
pairs A(dv .001) B(.005) C(.010) D(.030) E(.100, v0.1->0.2) x {price,delta,digital_OTM110}.
+ pair E digital strike scan (cal/eval 140/60) picking |s|_eval in [0.5,3].
d_ana: unified pair npz (A-D); pair E generated via Lewis (validated vs stored pair A d_ana).
Read-only on paper. No retrain.
"""
...
# --- Lewis d_ana for pair E: validate T against stored pair A d_ana ---
...
try:
    ...
    Tused,valerr,_=best
    print(f"[Lewis] pair A d_ana 검증: best T={Tused:.4f}, rel maxerr={valerr:.3f}")
    if valerr<0.05:
        pr={"v":0.100,"kappa":1.0,"omega":0.2,"xi":0.3,"rho":-0.5,"T":Tused}
        dana_E=lewis_pdf(pr,yg)-lewis_pdf(dict(pr,v=0.200),yg)
        print(f"[Lewis] pair E d_ana 생성 OK (T={Tused:.4f})")
    else:
        print(f"[Lewis] 검증 실패(err>{0.05}) → pair E S/J 생략")
except Exception as ex:
    print(f"[Lewis] 사용 불가: {ex} → pair E S/J 생략")
...
def emit(pair,kind,pname,g,p2,d_ana):
    ...
    if d_ana is not None:
        ...
        babs=abs(mu-Dtrue)  # |b_Delta| = |mu - Delta_true| (mu=Delta+b, Dtrue=Delta) approx
...
# pair E digital strike scan -> pick |s|_eval in [0.5,3]
...
for k,_ in picks:
    D=((p1-p2E)@((yg>k).astype(float))*dy)  # in-sample stats for consistency w/ ladder (all 200)
...
print("\n===== Part1 거리 사다리 (all 200 in-sample) =====")
...
print(f"\nCSV -> {OUT} ({len(rows)}행). (Jfrac/brel=-9 = d_ana 없음)")
