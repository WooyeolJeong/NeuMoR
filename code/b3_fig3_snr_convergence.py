"""
b3_fig3_snr_convergence.py — B.3: Fig 3 SNR convergence (unified formulation)
===============================================================================

Sweeps N_cal and measures SNR² gain at each N. Bootstrap 95% CI.

Heston: N_total=1000, N_cal=[50,100,150,200,300,500,700,950], K_test=seeds 950-999
Kou:    N_total=200,  N_cal=[30,50,75,100,125,150],            K_test=seeds 150-199

Outputs:
  NeuMoR/figures/b3_fig3_snr_convergence.{png,pdf}
  NeuMoR/output/unified/b3_snr_convergence.csv
  NeuMoR/output/unified/b3_asymptotic_gain.csv

Run:
  python NeuMoR/code/b3_fig3_snr_convergence.py
"""
...
for model, cfg in CONFIGS.items():
    ...
    for pair in ["A", "B", "C", "D"]:
        ...
        # d_hat from full cal set (fixed signal estimate)
        d_hat_full = npz["d_hat"]

        # B.2 asymptotic reference
...
for model in ["heston", "kou"]:
    for pair in ["A", "B", "C", "D"]:
        ...
        # Heston gain > Kou gain at all N
        if model == "heston":
            kou_sub = df[(df.model=="kou") & (df.pair==pair)]
            # only check at shared N values
