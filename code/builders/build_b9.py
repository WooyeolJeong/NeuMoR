"""
X1-CF (B9): main-suite fully cross-fitted recomputation.
================================================================================
Phase 1: run b5_thm35_387.main() ONCE (checkpoint inference) with measure_block
  monkeypatched to capture per-seed Delta_hat(g) scalars for all 387 rows.
  to_csv is no-op'd during main() so NO existing CSV is touched.
Phase 2: from the cached per-seed Delta_hat ONLY (no re-inference):
  - legacy recompute (sigma from evaluation/test) -> sanity gate (i) vs b5 ratio
  - V1 (sigma from calibration, single split)
  - V2 (R=20 random splits, ratio preserved, bidirectional swap-averaged)
Reuses b5.folded_normal_mean (no reimplementation). Writes only to output/b9_crossfit/.

Math (verified): build_k_eta -> sigma^2 = g@K@g dy^2 = ddof=0 sample variance of
Delta_hat_seed(g); compute_d_hat -> mu = mean_seed Delta_hat(g). So per-seed scalar
Delta_hat(g) is sufficient for mu_hat, sigma_hat, empirical.
"""
import sys, os, time
...
HERE = Path(__file__).resolve().parent           # .../NeuMoR/output/b9_crossfit
...
import b5_thm35_387 as b5   # light: module import (main() not called yet)
folded = b5.folded_normal_mean

# ---------------- Phase 1: inference + capture (skip if cache) ----------------
if NPZ.exists() and IDX.exists():
    ...
    pd.DataFrame.to_csv = lambda self, *a, **k: None    # protect existing CSVs
    t0 = time.time()
    try:
        b5.main()
    except SystemExit:
...
# ---------------- Phase 2a: legacy recompute + sanity gate (i) ----------------
...
for key in b5csv.key:
    ...
    ratio_leg = ratio_from(cal, test, test)      # mu=cal, sigma=TEST, emp=test  (= b5)
...
for key in sorted(b5csv.key):
    ...
    ratio_V1 = ratio_from(cal, cal, test)        # mu=cal, sigma=CAL, emp=test  (V1)
    # V2: R random splits, ratio preserved, bidirectional swap-averaged
    nseed = int(r.n_seeds); reps = np.zeros(R_V2)
    for j in range(R_V2):
        perm = rng.permutation(nseed); P = d[perm[:ncal]]; Q = d[perm[ncal:ncal + ntest]]
        r1 = ratio_from(P, P, Q)                  # P as cal (mu,sig), Q as test (emp)
        r2 = ratio_from(Q, Q, P)                  # swap: Q as cal, P as test
        reps[j] = 0.5 * (r1 + r2)
...
# sanity (ii): V1 out-of-band configs
