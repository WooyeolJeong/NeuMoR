"""
B12: regeneration of tab:efficiency (plug-in vs nonparametric estimation of E|Delta|).

(i)  Asymptotic columns  -- closed form eq:avar, evaluated with mpmath at dps=40.
       N sigma^-2 AVar(MR_np) = 1 + s^2 - f(s)^2
       N sigma^-2 AVar(MR_pi) = erf^2(s/sqrt2) + e^{-s^2}/pi
       f(s) = sqrt(2/pi) e^{-s^2/2} + s erf(s/sqrt2)
     Reported: variance ratio = AVar_pi/AVar_np, RMSE ratio = sqrt(variance ratio).

(ii) Finite-N columns -- independent Monte Carlo, R replications per cell.
     Ensemble x_1..x_N ~ N(s, 1) (sigma = 1, so mu = s).
       MR_np = mean |x_i|
       MR_pi = m(mu_hat, sigma_hat),  mu_hat = mean x,  sigma_hat = std(x, ddof=0)
     Target m(s, 1). Reported: MSE(MR_pi)/MSE(MR_np) with per-cell MC standard error
     from the delta method on the ratio of two dependent means.

     ddof=0 (MLE) matches the estimator definition used in the empirical suite.

Outputs: tab_efficiency.csv  (one row per (s, N) cell plus asymptotic columns per s)
Read-only with respect to every other artifact. Writes only to output/b12_efficiency/.
"""
import os
...
def m_np(mu, sigma):
    """folded-normal mean, float path (for the simulation)."""
    z = mu / sigma
