"""
X2-LS (B10): low-SNR targeted panel, fully out-of-sample cross-fit.
================================================================================
Phase 1 (inference ONCE): Heston Protocol A, 200 seeds, 4 distinct lambda points
  (lambda1 = BASE shared by all 3 pairs; three lambda2). Per-seed density fields
  are written to cache/ and never recomputed.
Phase 2 (cache-only): cell construction + V1/V2 + bin fill + sanity + CSV.

Pairs (approved gate 0):
  P1 Exp01-C          v      0.100 -> 0.110            ||dlam|| = 0.0100
  P3 Exp05dg-all5_s4  5-axis diagonal, scale 4         ||dlam|| = 0.0472
  P2 Exp05ax-kappa4   kappa  1.000 -> 1.300            ||dlam|| = 0.3000

Cells per pair:
  [R] g(th,k) = cos(th)*g_sig + sin(th)*v_k,  g_sig = d_cal/||d_cal||,
      v_k = k-th leading eigvec of K_cal, Gram-Schmidt orthogonalized to d_cal.
      th solved (brentq) from target |s_cal| in {.05,.15,.25,.4,.6,.8,1.0} for
      k=1,2, plus th=0 anchor.  -> 15 cells/pair
  [I] butterfly centered at y0 (3 widths) + digital (2 strikes)  -> 5 cells/pair

y0 rule (pre-approved):
  y0 = sign crossing of d_cal (150-cal split) within |y|<=2 maximizing |d_cal'|.
  fallback: argmin |d_cal| in window, knob flagged no-crossing.

V1: mu_hat, sigma_hat  <- calibration seeds 0..149 of Delta_hat(g)
    empirical          <- mean |Delta_hat(g)| over evaluation seeds 150..199
V2: R=20 random 150/50 splits (RNG 20260827), shared across all cells.

Reuses build_k_eta / compute_d_hat / effective_rank (neumor_core) and
b5.folded_normal_mean. No reimplementation. Writes only to output/b10_lowsnr/.

Math note (same identity as B9): with K = build_k_eta(l1_cal, l2_cal),
g@K@g*dy^2 == ddof=0 sample variance of per-seed Delta_hat(g), and
compute_d_hat gives mu = mean_seed Delta_hat(g). So the per-seed scalar
Delta_hat(g) = (l1-l2) @ g * dy is sufficient for mu_hat, sigma_hat, empirical.
This equivalence is asserted numerically at runtime (gate EQ).
"""
...
HERE = Path(__file__).resolve().parent            # .../NeuMoR/output/b10_lowsnr
NEUMOR = HERE.parents[1]                          # .../NeuMoR
ROOT = NEUMOR.parent                              # import root
CODE = NEUMOR / "code"
CACHE = HERE / "cache"
...
# shared V2 splits (identical across every cell -> paired comparison)
...
# equivalence gate: g@K@g dy^2 == var(Delta_hat) ddof=0
eqmax = 0.0
for t in P:
    p = P[t]
