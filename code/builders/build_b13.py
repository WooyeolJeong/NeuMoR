# -*- coding: utf-8 -*-
"""
build_b13.py — regeneration of tab:sup-proxy-sensitivity (vanilla-basis truncation sweep).

The original sweep script was not preserved; this is a RECONSTRUCTION of the procedure,
not a recovered file. Basis: manuscript eq:proj-A (A = K + tau I) and the table caption
      (relative truncated-SVD: discard sigma_i <= t*sigma_max),
      and the recorded sweep settings tau = 0.01*lam_max, t in {1e-12..1e-4}.

Reads:  {model}_pair{A..D}.npz  (8 files, read-only)
Writes: output/b13_supproxy/ only
No checkpoints, no torch, no retraining.
"""
...
NEU  = HERE.parents[1]                 # .../NeuMoR
ROOT = NEU.parent                      # .../Transition Neural Operator
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(NEU / "code"))
from NeuMoR.code.neumor_core import snr_squared          # noqa: E402
from b8_functions import build_basis                     # noqa: E402
...
# Printed table values, for comparison (manuscript Table: sup-proxy sensitivity)
PAPER = {
...
# Gate reference values (verify_aproj.csv)
...
# Gate reference values (verify_loss.csv, recA_oos)
...
for model in MODELS:
    for pair in PAIRS:
        ...
        # reference SNR^2 (tau-free, held-out)
