"""
F1-SIM step 7b: tree per-cell overlay from STORED cross_class RF/GB density arrays.
Array ops only (no retrain / no re-inference). Appends source=tree_percell rows to
b11_overlay.csv. Config set = cross_class near-pair set (NOT b5 387) -> caveat.
Per cell: s=|mu_hat|/sigma_hat, gamma1/2 of per-seed Delta_hat(g), observed ratio,
Edgeworth-predicted ratio. |y|<=1.5 window (avoids tail-degenerate cells).
"""
...
sys.path.insert(0, str(NEUMOR/"code")); import b5_thm35_387 as b5
...
for name,y in SRC:
    ...
    w=trapw(y)[gps]; P=payoff_mat(y)[:,gps]                        # (npay, ngp)
    Vg=np.einsum('scg,pg->scp', M[:,:,gps], P*w[None,:])           # (nseed,ncfg,npay)
    near=near_pairs(L)
    for (a,bb) in near:
        for pi,pl in enumerate(PAYS):
            d=Vg[:,a,pi]-Vg[:,bb,pi]                               # (nseed,) per-seed Delta_hat(g)
            mu=float(d.mean()); sg=float(d.std(ddof=0))
            if not (sg>0 and np.isfinite(mu)): continue
            s=abs(mu)/sg
            if s>5 or s<1e-3: continue                             # match sim s-range
...
# 7c aggregate anchors (documented block-level; per-cell NOT) — kept as separate labeled rows
...
# append to existing overlay (7a rows), preserving them
...
# overlay summary per source (tree)
def summ(df):
...
# tree_percell binned by gamma2 (edge validity regime)
if len(tp):
