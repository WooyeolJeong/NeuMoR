"""
X2-LS (B10) addendum: I2 family — DIGITAL SPREADS at the cumulative-signal zero.

Why not a single digital (measured, retained as a negative result):
for g_K = 1{y>K} the signal is mu(K) = F(K) = sum_{y_i>K} d_cal_i * dy, which moves
in quantized steps of |d_cal(K)|*dy as K crosses grid points. The zero of F sits where
d_cal changes sign, i.e. exactly where |d_cal| is LARGE, so the quantization is coarsest
right where fine control is needed. Measured achievable floor over the 120 in-window
digitals: |s| >= 0.1514 (P1), 0.6061 (P2), 0.7594 (P3) — the target grid
{0.05,0.15,0.3,0.6,1.0} is unreachable. Additionally the |s| minimum for P2 landed at
the window edge K=-1.99, where 1{y>-1.99} ~ 1 and mu ~ int d_cal ~ 0 holds trivially by
normalization (a degenerate "buy everything" cell), not by any cancellation.

Fix: a spread g = 1{K1 < y <= K2} = 1{y>K1} - 1{y>K2} is the DIFFERENCE of two digitals,
so mu = F(K1) - F(K2) exactly (asserted at runtime, gate SP). Two dials — K1 coarse,
K2 fine — make the attainable mu set combinatorial (pairwise differences of the 120
step values) rather than a single ladder, which removes the quantization floor. A call
spread is also a more standard traded instrument than a bare digital.

Screens (both applied): mass int g*pbar_cal dy >= 0.02 ; width K2-K1 >= 3*dy.
Matching: nearest achievable |s_cal| to each target, ACCEPTED ONLY IF
|s_achieved - s_target| <= 0.05. Otherwise the target is reported unmet for that pair.
No forcing.

CACHE-ONLY: no inference. Existing 72 rows of b10_lowsnr_all.csv are preserved
byte-for-byte by pure text append. summary + verdict.json I2 fields regenerated;
existing verdict PASS fields untouched.
"""
import sys, os, json
...
for tag, (k1k, k2k, cid, blk) in PAIRKEY.items():
    ...
    # g = 1{y[a] < y <= y[b]}  -> indices a+1 .. b
    delta = (S[:, B + 1] - S[:, A + 1]) * dy       # (200, n_combo)
...
# ---- text append: existing 72 rows untouched byte-for-byte ----
...
print("\n===== b10_lowsnr_summary.csv (I2 스프레드 포함, 재생성) =====")
