"""
F1-SIM (B11): controlled verification of the non-Gaussian theory pieces.
================================================================================
(1) W1 bound  |E|mu+eta| - m(mu,sigma)| <= W1(Law(eta), N(0,sigma^2))   [sigma=1]
(2) Edgeworth 1st-order correction direction/size + validity map
(7a) overlay: NN 387-suite (small gamma2) + b10 X2 (null) vs Edgeworth-predicted ratio
Pure numerical integration (scipy.quad) + analytic standardization. NN/manuscript untouched.
folded_normal_mean reused from b5 (NO reimplementation; closed forms cross-check only).
Writes only to output/b11_nongauss/.
"""
...
folded = b5.folded_normal_mean                       # reused (no reimpl)
...
def kink_quad(fn, mu):
    """int_{-inf}^{inf} fn(z) dz with a kink at z=-mu (split there)."""
...
def W1_to_normal(cdf, atom_at=None):
    """W1(eta, N(0,1)) = int |F_eta(x) - Phi(x)| dx.  Split at 0 (+ atom breakpoint)."""
...
# ---------------- distribution families (mean 0, var 1 analytically) ----------------
def make_t(nu):
    sc = np.sqrt((nu - 2.0) / nu)                     # var(t)=nu/(nu-2) -> Y=t*sc has var 1
...
def make_zi(p):
    sig = 1.0/np.sqrt(p)                               # continuous part N(0,1/p); var=p*(1/p)=1
    return dict(name=f"ZI{p}", g1=0.0, g2=3.0/p - 3.0, p=p, sig=sig, atom=0.0,
                pdf=lambda z: p*norm.pdf(z, scale=sig),   # continuous density (atom handled separately)
                cdf=lambda x: (1-p)*(x >= 0) + p*norm.cdf(x, scale=sig),
                E_true=lambda mu: (1-p)*abs(mu) + p*folded(mu, sig),
                E0_closed=(0.0*0 + (1-p)*0 + p*folded(0.0, sig)))   # =p*sig*sqrt(2/pi)
...
for fam in FAMILIES:
    ...
    for s in S_GRID:
        mu = float(s)
        if "E_true" in fam:                            # ZI: analytic atom + continuous
...
G3_ET_TGT = np.sqrt(0.5)              # 1/sqrt(2), full precision
G3_EE_TGT = S2PI * (1 - 3/24)          # sqrt(2/pi)*(1-gamma2/24)
...
for k in [2, 8]:
    ...
    # sign of split at s=1 should match sign of gamma1*C3 term
...
# ---------------- step 6: kurtosis sufficiency (gamma2=3 families) ----------------
suff_rows = []
...
# edge_valid boundary: smallest gamma2 where edge_valid fails somewhere in s<=1
inval = cells[(~cells.edge_valid) & (cells.s <= 1.0)]
edge_valid_boundary = float(inval.gamma2.min()) if len(inval) else None

# ---------------- 7a overlay: NN 387-suite + b10 null ----------------
...
ov = []
# NN suite: b5_thm35_all (s, ratio) join per_config_gaussianity_v2 (kurt, skew)
...
# kurt convention: NN ~ Gaussian => raw(ref 3) if median near 3. Verify:
